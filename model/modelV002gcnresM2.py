...
class CROG(nn.Module):
    def __init__(self, cfg):
        ...
        self.num_nodes = 31
        self.gcn_weight = cfg.gcn_weight
    ...
    def visualize_bbox_cv2(self, vis, bboxes):
        """
        使用 OpenCV 可视化 bboxes 在 vis 中的位置
        """
        # 转换特征图为可视化图像
        vis_img = vis[0].mean(dim=0).detach().cpu().numpy()  # 平均通道，获得单通道特征图
        vis_img = (vis_img - vis_img.min()) / (vis_img.max() - vis_img.min()) * 255  # 归一化到 [0, 255]
        vis_img = vis_img.astype(np.uint8)  # 转为 uint8 类型
        vis_img = cv2.cvtColor(vis_img, cv2.COLOR_GRAY2BGR)  # 转为 BGR 格式以绘制彩色边框

        # 绘制边界框
        for bbox in bboxes:
            x1, y1, x2, y2 = bbox
            scale_x, scale_y = 52 / 640, 52 / 480
            x1, x2 = int(x1 * scale_x), int(x2 * scale_x)
            y1, y2 = int(y1 * scale_y), int(y2 * scale_y)

            # 绘制矩形边框
            cv2.rectangle(vis_img, (x1, y1), (x2, y2), (0, 0, 255), 2)  # 红色边框，厚度为 2

        # 显示或保存结果
        cv2.imshow("Feature Map with BBoxes", vis_img)
        cv2.waitKey(0)  # 按任意键关闭窗口
        cv2.destroyAllWindows()
        # 如果需要保存结果，可使用以下代码：
        # cv2.imwrite("feature_map_with_bboxes.png", vis_img)
    def apply_attention_to_vis(self, feat_img, gcn_output, bboxes):
        """
        ...
        for i, bbox in enumerate(bboxes):
            ...
            updated_region = feat_img[:, y1:y2, x1:x2] * (1 + attention_map * self.gcn_weight)
            feat_img = feat_img.clone()

            feat_img[:, y1:y2, x1:x2] = updated_region

            # self.visualize_bbox_cv2(vis[0], bboxes)
    ...
    def forward(self, img, imgdet, word, mask=None, grasp_qua_mask=None, grasp_sin_mask=None, grasp_cos_mask=None, grasp_wid_mask=None):
        ...
        for i in range(imgdet.size(0)):
            ...
            updated_feat = self.apply_attention_to_vis(vis[0][i], gcn_output, bboxes)
            updated_vis.append(updated_feat)
        vis_list = list(vis)
        vis_list[0] = torch.stack(updated_vis, dim=0)
        vis_list[0] = vis_list[0] + vis_res

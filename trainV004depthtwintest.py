...
import wandb
from utils.datasetV004depthtext import OCIDVLGDataset

from utils.misc import (init_random_seed, set_random_seed, setup_logger,
                        worker_init_fn)
from model.modelV004depthtwintext import CROG_TWIN
...
def train_with_grasp(train_loader, model, optimizer, scheduler, scaler, epoch, args):
    ...
    for i, data in enumerate(train_loader):
        ...
        # forward
        with amp.autocast():
            pred, target, loss, loss_dict = model(image, imgdet,imgdepth, text, ins_mask, grasp_qua_mask, grasp_sin_mask,
...
def validate_with_grasp(val_loader, model, epoch, args):
    ...
    for data in pbar:
        ...
        pred, target = model(image, imgdet,imgdepth, text, ins_mask, grasp_qua_mask, grasp_sin_mask, grasp_cos_mask,
                             grasp_wid_mask)
...
def get_parser():
    ...
    parser.add_argument('--config',
                        default='/home/lbycdy/work/VGCGM/config/OCID-VLG/20241126modelV003depth0.25twin.yaml',
                        type=str,
                        help='config file')

...
import nltk
nltk.data.path.append('/home/lbycdy/work/VGCGM/NLTK')
from nltk.tokenize import word_tokenize
from nltk import pos_tag
# tokens = word_tokenize('Grasp the right pear.')
# print(tokens)
...
#     if tag in ['NN', 'NNS', 'NNP', 'NNPS']:  # 名词相关标记
...
class RefOCIDGraspDataset(Dataset):
    ...
    def convert(self, img, mask=None, grasp_quality_mask=None, grasp_sin_masks=None, grasp_cos_masks=None,
                grasp_width_mask=None):
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            ...
            # )
            return rgb, (
            ins_masks, grasp_quality_masks, grasp_sin_masks, grasp_cos_masks, grasp_width_masks), word_vec, params
        elif self.mode == 'test':
            rgb = self.convert(rgb)[0]
            word_vec = tokenize(sentence, self.word_length, True).squeeze(0)
            params = {
...
class OCIDVLGDataset(Dataset):
    ...
    def collate_fn(batch):
        return {
            ...
            "imgdet": torch.stack([torch.from_numpy(x['imgdet']) for x in batch]),
